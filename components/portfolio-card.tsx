    ...
    <div className="space-y-4">
      ...
      <div className="bg-yellow-400 rounded-2xl p-6">
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            <p className="text-black text-sm font-medium">Receive</p>
            <p className="text-black/70 text-xs">05.12.2024</p>
          </div>
        </div>
        <div className="ml-11">
          <p className="text-black text-lg font-bold">+590.41</p>
          <p className="text-black/70 text-sm">MATIC</p>
        </div>
        <p className="text-black/70 text-xs mt-2">◐ Confirmed</p>
      </div> */}

      {/* Additional transactions */}
      {/* <div className="space-y-2">
        {sendTxns.map((txn) => (
          <div key={txn.id} className="bg-neutral-950 border border-neutral-800 rounded-xl p-3 flex items-center gap-3">
            <div className="text-lg">{txn.icon}</div>
            <div className="flex-1">
              <p className="text-white text-xs font-medium">{txn.type}</p>
              <p className="text-neutral-500 text-xs">{txn.time}</p>
            </div>
            <p className="text-neutral-300 text-xs font-medium">{txn.coin}</p>
          </div>
        ))}
      </div> */}
    </div>
